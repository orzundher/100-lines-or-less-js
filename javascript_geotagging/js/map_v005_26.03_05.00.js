...
function save(id) {
	...
	if (o<10) a=18; else a=0;
	buffer = new ArrayBuffer(bFile.getLength()+o-18+234-a), 
		data = new DataView(buffer);
	for (var i = 0;i<2;i++)	data.setUint8(i,model.getByteAt(i));
	for (var i = a+2;i<234;i++) data.setUint8(i-a,model.getByteAt(i));
	data.setUint16(22-a,offset+234-o-18-a); 		
	for (var j=0;j < 2;j++) { 
		data.setUint8(j*8+189-a,parseInt(latitude[j]));
		data.setUint8(j*8+213-a,parseInt(longitude[j]));}
	data.setUint16(204-a,parseInt(latitude[2]*1000));
	data.setUint16(208-a,parseInt(1000));
	data.setUint16(228-a,parseInt(longitude[2]*1000));
	data.setUint16(232-a,parseInt(1000));
	data.setUint8(142-a,latitude[3].charCodeAt(0));
	data.setUint8(166-a,longitude[3].charCodeAt(0));
	for (var i = o+18; i<bFile.getLength(); i++) 
		data.setUint8(i-o-18+234-a,bFile.getByteAt(i)); 
